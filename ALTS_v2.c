 ...
 */
void Process_Tree_22June23(int tree[][TWO], int edges_no, int no_nodes,
 int ch[][MAX_OUT]){
    int  t, k;
...
void Process_Tree(int tree[][TWO], int edges_no, int ch[][MAX_OUT], int pa[]){
    int  t, k;
    int max_nodes_id;
...
/* used */
/* gph is a tree */
void  Simplify_Ntks_Initialization(int gph[][2], int no_edges,
   int no_nodes, int leaf_map[], int current_comp,  int last){
    int i, j, k, r;
...
void Compute_Paritions(int no_tree_nodes, int no_lf, int sorted_nodes[], 
...
/* used */
void Update(int no_tree_nodes, int no_lfs, char *all_zeros, int  common_part_no[], 
...
void Remove_part(int no_lfs, char cluster2[], char cluster1[]){
    int i;
...
int  Contain_Cluster(int no_lfs, char *cluster1, char *cluster2){
    int i;
...
int  Construct_Subtrees(int no_t_nodes, int no_lfs, int subtree[][2], 
...
void top_sort_Dec20(int graph[][2], int no_ntk_nodes, 
 int no_ntk_edges, int sorted_nodes[]){
    int i, j, k;
    int  *in_deg=NULL, *ot_deg=NULL, *roots=NULL;
   int (*child)[MAX_OUT];
...
void Common_Clusters(int no_trees, int Input_trees[][NO_NODES][2], 
    int no_tree_nodes,  int no_tree_edges, int no_lfs,
    ...
    for (i=0; i<(int) no_trees; ) {
     /*  renumber nodes int: root=0; internal nodes are numbered from lf_lo+2; */
     Simplify_Ntks(Input_trees[i], no_tree_edges, no_tree_nodes);
     /* compute child and parent relationship: t_ch and t_p */
     Process_Tree_22June23(Input_trees[i], no_tree_edges,  no_tree_nodes, 
            tree_ch[i]);
     i++;
   }


   all_zeros=(char *)malloc(no_lfs+2);
...
/*
void Print_subtrees(int tree[][2], int subtree_size){
   int i;
 ...
 */
void  Compute_ComponentGph(int no_trees, int no_tree_nodes, int no_lfs, 
...
void Adjust_map(int tree[][2], int ind,  int no_lfs, 
  int leaf_mapping[][NO_LEAVES+1], char *node_clusters[][NO_NODES]){
   
     int map[NO_LEAVES]; /*  map[j]=i meaning jth leaf in tree[j]=the jth leaf of tree[0] */
...
void   Decompose_Trees_Into_Componets(int no_trees, int I_trees[][NO_NODES][2],
   int I_node_no, int I_edge_no, int I_lf_no, char *node_clusters[][NO_NODES], 
...
int Equal_Containment(int list1[], int list2[]){
   int k;
...
int  Equality_Contain_str(char *seqs_str[], int k,  char *str){ 
     int i;
 ...
 * do multiple sequence alignemtn in the limited space */
  ...
  /* important. if not removed, fails on ap. */
   /*
     for (i=0; i<dim1; i++) { 
       if (seqs_str3[i]) free(seqs_str3[i]);
     }  
   */
...
void Convert_Str_Format_July30(char *inci_str[], int taxa_inci[], int lf_no,
 int p[]) {
    int i; 
    int pa, j;
    char *str;
...
int  Compute_tree(char* nw_str, int tree[NO_NODES][2],  char *taxa_names[], int index) {
    char a[TAXA_NAME_STR] = { 0 };
...
int Extract_Taxa(char *newick_str, char *names[]){
    int i;
...
int  Convert_to_Edges(int flag, char *newick_str, char *taxa_names[],
    int tree[][2], int *lfs_no) {

    int i = 0, index, p=0, len, edge_no;
    char no_internalID_format[TREE_INPUT_STR];
    char id_str[TAXA_NAME_STR];
...
void Add_Rootedge(int edge_no, int  tree[][2]){
   int node_no;
   int *in_deg, *out_deg;
   int i, root;

   in_deg=(int *)malloc((edge_no+2)*sizeof(int));
   out_deg=(int *)malloc((edge_no+2)*sizeof(int));
...
/* INT_MAX=2,147,483,647 */
 ...
 for (i=0; i<NO_TREES; i++) {
     for (j=0; j<NO_NODES; j++) {
         node_clusters[i][j]=(char *)malloc((I_node_no/2)+2);
          sorted_tnodes[i][j]=0;
  ...
  for (node_ind=I_node_no-1; node_ind>=1; node_ind--){
   ...
   if (ComponentGraph[sorted_tnodes[0][node_ind]][0]>0){
     ...
     for (j=0; j<no_trees; j++){
       ...
       t_p[j]=(int *)malloc(sizeof(int)*(subtr_edge_no+2));

       Simplify_Ntks_Initialization(subtrees[j], subtr_edge_no,  
         subtr_edge_no+1, leaf_mapping[j], current_comp, no_components);
       ...
       Process_Tree(subtrees[j], subtr_edge_no,   t_ch[j], t_p[j]);
